...
const CreatePost = () => {
  ...
  const addImageToPost = (e) => {
    ...
    if(e.target.files[0]) {
      reader.readAsDataURL(e.target.fiels[0]);
      reader.onload = (e) => {
    ...
    <div className='bg-white rounded-md shadow-md text-gray-500 p-2'>
      ...
      <div className='flex justify-evenly py-2'>
        <div className='flex items-center p-1 space-x-1 flex-grow justify-center hover:bg-gray-100 rounded-md hover:cursor-pointer'>
          ...
          <p className='font-semibold text-gray-699'>Live Video</p>
        </div>
        <div onClick={handleClick} className='flex items-center p-1 space-x-1 flex-grow justify-center hover:bg-gray-100 rounded-md hover:cursor-pointer'>
          <IoMdPhotos onChange={addImageToPost} size={20} className='text-green-500'/>
          <p className='font-semibold text-gray-699'>Photo/Video</p>
          <input type='file' ref={hiddenFileInput} hidden accept='image/*'/>
        </div>
        <div className='flex items-center p-1 space-x-1 flex-grow justify-center hover:bg-gray-100 rounded-md hover:cursor-pointer'>
          <BsEmojiSmile size={20} className='text-yellow-500'/>
          <p className='font-semibold text-gray-699'>Feeling/Activity</p>
        </div>
      </div>
    </div>
  )
